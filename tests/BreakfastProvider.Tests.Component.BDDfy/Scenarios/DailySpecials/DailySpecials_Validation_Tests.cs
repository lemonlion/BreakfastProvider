...
public class DailySpecials_Validation_Tests : BaseFixture
{
    ...
    {
        this.Given(x => x.A_daily_special_order_request_with_invalid_field(field, value, reason), "A daily special order request with invalid {0}")
    ...
    private void A_daily_special_order_request_with_invalid_field(string field, string? value, string reason)
    {
