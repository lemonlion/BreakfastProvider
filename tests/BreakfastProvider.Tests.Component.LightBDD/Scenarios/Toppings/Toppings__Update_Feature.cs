...
public partial class Toppings__Update_Feature
{
        ...
        TabularInputs<InvalidFieldFromRequest> inputs, TabularOutputs<VerifiableErrorResult> outputs)
    {
        await Runner.RunScenarioAsync(
            given => A_known_topping_exists(),
            and => Valid_update_topping_requests_with_an_invalid_field(inputs),
            when => The_invalid_update_topping_requests_are_submitted(),
            then => The_update_responses_should_each_contain_the_validation_error_for_the_invalid_field(outputs));
    }
