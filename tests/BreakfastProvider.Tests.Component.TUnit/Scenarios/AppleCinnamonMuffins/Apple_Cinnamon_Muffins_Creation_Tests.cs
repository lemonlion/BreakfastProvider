...
namespace BreakfastProvider.Tests.Component.TUnit.Scenarios.AppleCinnamonMuffins;
...
public class Apple_Cinnamon_Muffins_Creation_Tests : BaseFixture
{
    ...
    [Test]
    [MethodDataSource(typeof(MuffinRecipeVariations), nameof(MuffinRecipeVariations.RecipeVariations))]
        ...
        string recipeName, MuffinRecipeTestData recipe, int temperature, int durationMinutes, string panType, MuffinBatchExpectation expected)
    {
        ...
        var bakingTemperatureMatchesExpectation = expected.HasBakingInfo
            ...
            : _muffinSteps.Response!.BakingTemperature == 0;
        await bakingTemperatureMatchesExpectation.Should().BeTrue();
    }

    [Test]
    [Arguments("Flour", "", "Flour is required", "'Flour' is required.", "Bad Request")]
    [Arguments("Apples", "", "Apples is required", "'Apples' is required.", "Bad Request")]
    [Arguments("Cinnamon", "", "Cinnamon is required", "'Cinnamon' is required.", "Bad Request")]
    [Arguments("Milk", "", "Milk is required", "'Milk' is required.", "Bad Request")]
    [Arguments("Eggs", "", "Eggs is required", "'Eggs' is required.", "Bad Request")]
    [Arguments("Cinnamon", "<script>alert('xss')</script>", "XSS in cinnamon", "Cinnamon contains potentially dangerous content.", "Bad Request")]
