...
public class Orders_Status_Transition_Tests : BaseFixture
{
    ...
    [Test]
    [Arguments("Created", "Preparing")]
    [Arguments("Created", "Cancelled")]
    [Arguments("Preparing", "Ready")]
    [Arguments("Ready", "Completed")]
    public async Task Valid_status_transition_should_update_the_order(string fromStatus, string toStatus)
    {
        ...
        await _patchSteps.Response!.Status.Should().BeEqualTo(toStatus);
    }

    [Test]
    [Arguments("Created", "Ready")]
    [Arguments("Created", "Completed")]
    [Arguments("Preparing", "Cancelled")]
    [Arguments("Ready", "Preparing")]
    [Arguments("Completed", "Preparing")]
    [Arguments("Cancelled", "Preparing")]
    [Arguments("Cancelled", "Ready")]
    public async Task Invalid_status_transition_should_return_conflict(string fromStatus, string toStatus)
    {
        ...
        await _patchSteps.ResponseMessage!.StatusCode.Should().BeEqualTo(HttpStatusCode.Conflict);
    }
}
